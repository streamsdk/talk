...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        icon = [[NSArray alloc]initWithObjects: @"photog150.png",@"camera_icon150.png",@"video150.png",@"map.png",nil];
    }
    return self;
}
...
{
	//row number
    NSArray * array = [[NSArray alloc]initWithObjects:@"Choose a Photo",@"Camera",@"Choose a Video",@"location", nil];
	for (int i=0; i<1; i++) {
		//column numer
		for (int y=0; y<4; y++) {
